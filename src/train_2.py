...
class ImprovedSymbolicDataset(Dataset):
    ...
    def generate_data(self, problems, num_iterations=10000, max_atoms=15, cache_file="data/trajectories_cache_v2.pt"):
        import os
        ...
        print(f"Generating optimized MCTS trajectories for {len(problems)} equations...")
        ...
        for seq in synthetic_sequences:
            for _ in range(multiplier):
                for i in range(len(seq)):
                    ...
                    self.data.append((
                        torch.tensor(seq_indices, dtype=torch.long),
                        torch.tensor([1.0], dtype=torch.float32), # High confidence/value
                        torch.tensor(policy, dtype=torch.float32)
                    ))
        print(f"  -> Added {len(synthetic_sequences) * multiplier} synthetic trajectories constraints.")
...
def train():
    import argparse
    parser = argparse.ArgumentParser(description="Train AlphaZero PredictorNN - Phase 2")
    ...
    if os.path.exists(prev_ckpt):
        ...
        model.load_state_dict(torch.load(prev_ckpt, map_location="cpu", weights_only=True))
    else:
    ...
    print(f" -> Best Checkpoint Phase 2 saved to {checkpoint_dir / 'predictor_v2_best.pt'} (loss: {best_loss:.4f})")
