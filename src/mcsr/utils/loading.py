...
REPO_NAME = "yoshitomo-matsubara/srsd-feynman_easy"
...
class SRSDLoader:
    ...
    def __init__(self, splits: Iterable[str]) -> None:
        ...
        self.splits = tuple(split.lower() for split in splits)
        if not all(split in ["train", "validation", "test"] for split in self.splits):
            raise ValueError("Splits must be one of 'train', 'validation', or 'test'.")
...
def load_true_sympy_expressions() -> dict[str, sympy.Expr]:
...
def load_pickled_expressions(path: str | Path) -> dict[str, Expression]:
    equations = {}
    for file_path in Path(path).glob("*.pkl"):
        with open(file_path, "rb") as f:
            equations[file_path.stem] = pickle.load(f)
    return equations
