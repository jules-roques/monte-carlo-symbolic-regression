...
def atom_key(a: Atom) -> str:
    if isinstance(a, Constant): return f"const_{a.value}"
    if isinstance(a, Variable): return f"var_{a.var_index}"
    return f"op_{a.name}"

class MutatorInterface(Protocol):
    def mutate(self, expression: Expression, num_mutations: int, grammar: Optional[Grammar] = None) -> list[tuple[Expression, float]]:
        """
...
class MutatorNN(nn.Module, MutatorInterface):
    ...
    """
    def __init__(self, grammar: Optional[Grammar] = None, max_atoms: int = 15, embedding_dim: int = 64, hidden_dim: int = 128):
        super().__init__()
    ...
    def forward(self, parent_sequences: torch.Tensor, child_sequences: torch.Tensor) -> torch.Tensor:
        """
        ...
        start_tokens = torch.full((batch_size, 1), self.vocab_size, dtype=torch.long, device=device)
    ...
    def mutate(self, expression: Expression, num_mutations: int, grammar: Optional[Grammar] = None) -> list[tuple[Expression, float]]:
        ...
        for _ in range(num_mutations):
            ...
            while remaining_leaves > 0:
                ...
                valid_atoms = sampling_grammar.get_valid_atoms(remaining_leaves, self.max_atoms, len(seq))
                if not valid_atoms:
                    valid_atoms = sampling_grammar.terminal_atoms
                    
                valid_indices = [self.atom_to_idx[atom_key(a)] for a in valid_atoms if atom_key(a) in self.atom_to_idx]
    ...
    def mutate(self, expression: Expression, num_mutations: int, grammar: Optional[Grammar] = None) -> list[tuple[Expression, float]]:
        ...
        for _ in range(num_mutations):
            ...
            while leaves > 0 and len(seq) < self.max_atoms:
                valid = sampling_grammar.get_valid_atoms(leaves, self.max_atoms, len(seq))
                if not valid: valid = sampling_grammar.terminal_atoms
