...
from mcsr.utils.mutator import MutatorInterface, DummyMutator
...
def _ucb_score(child: DGSRNode, parent_visit_count: int, exploration_constant: float) -> float:
    if child.visit_count == 0:
        return float("inf")
    # Equation du MCTS : Q(s,a) + c * sqrt(ln(N(s)) / N(s,a))
    return child.mean_score + exploration_constant * math.sqrt(math.log(parent_visit_count) / child.visit_count)

class DGSR_MCTS(ResearchAlgoInterface):
    ...
    def __init__(
        ...
        if mutator is not None:
            ...
            self.mutator = MutatorNN(max_atoms=max_atoms).to(device)
            print(f"Loading MutatorNN from {mutator_path} ({device})...")
            checkpoint = torch.load(mutator_path, map_location=device, weights_only=True)
    ...
    def _evaluate(self, expr: Expression, input_data: np.ndarray, target: np.ndarray) -> float:
        """Evalue une expression avec MSE/NRMSE moins la pénalité de complexité."""
        try:
            predicted = expr.evaluate(input_data)
    ...
    def fit(self, input_data: np.ndarray, target: np.ndarray) -> tuple[Expression, float]:
        import random
        ...
        for _ in range(self.num_iterations):
            ...
            if not node.is_expanded:
                ...
                mutations = self.mutator.mutate(node.expression, self.num_mutations_per_expansion, grammar=self.grammar)
                
                # Create children corresponding to mutations
                for expr, log_prob in mutations:
                    child = DGSRNode(expression=expr, parent=node, prior=math.exp(log_prob))
