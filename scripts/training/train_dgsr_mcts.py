...
from mcsr.algos.DGSR import DGSR
...
def prepare_batch(trajectories, mutator, device):
    ...
    for p_expr, c_expr, r in trajectories:
        p_idx = [
            mutator.atom_to_idx.get(atom_key(a), vocab_size)
            for a in p_expr.atom_sequence
        ]
        c_idx = [
            mutator.atom_to_idx.get(atom_key(a), vocab_size)
            for a in c_expr.atom_sequence
        ]
    ...
    # Padding
    p_padded = nn.utils.rnn.pad_sequence(
        parents, batch_first=True, padding_value=vocab_size
    )
    c_padded = nn.utils.rnn.pad_sequence(
        children, batch_first=True, padding_value=vocab_size
    )
...
def train_DGSR():
    ...
    print(
        f"Début de l'entraînement : {num_epochs} époques, {mcts_iters} itérations MCTS par problème."
    )
    ...
    for epoch in range(num_epochs):
        ...
        for prob_id in range(num_problems):
            ...
            best_expr = algo.fit(x, y)
            best_score = algo._evaluate(best_expr, x, y)
        ...
        if torch.isnan(loss) or torch.isinf(loss):
            print(
                f"Epoch {epoch + 1} - ALERTE: loss est {loss.item()}, on saute l'update."
            )
            continue
        ...
        print(
            f"Epoch {epoch + 1}/{num_epochs} | Traj: {len(all_trajectories)} | Loss: {loss.item():.4f} | Avg Best: {avg_score:.4f} | Baseline: {avg_r:.4f}"
        )
    ...
    torch.save(
        ...
        checkpoint_dir / "mutator_final.pt",
    )
    print(
        f"Entraînement terminé. Modèle sauvé dans {checkpoint_dir / 'mutator_final.pt'}"
    )
