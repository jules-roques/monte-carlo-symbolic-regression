...
class RealSymbolicDataset(Dataset):
    ...
    def generate_data(
        ...
        max_atoms=15,
        cache_file="data/trajectories_cache.pt",
    ):
        ...
        print(
            f"Generating real MCTS trajectories for {len(problems)} held-out equations..."
        )
...
def train():
    ...
    loading = DataLoader(dataset, batch_size=64, shuffle=True, collate_fn=collate_fn)
    ...
    epochs = args.epochs
    print(
        f"Starting Machine Learning Phase on {len(dataset)} trajectories for {epochs} epochs..."
    )
    best_loss = float("inf")
    ...
    print(
        f" -> Best Checkpoint saved to {checkpoint_dir / 'predictor_epoch_best.pt'} (loss: {best_loss:.4f})"
    )
