...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Difficulty of the equations to solve (different benchmarks)",
    )
    parser.add_argument(
        "--pickles_path",
        type=str,
...
def process_all_equations(model: SRAlgorithm, loader: SRSDLoader) -> list[dict]:
    ...
    for equation_data in tqdm(
        loader,
        desc=f"Training {model.__class__.__name__} on {loader.difficulty} equations",
    ):
...
def print_results_table(model_name: str, difficulty: str, results: list[dict]) -> None:
    ...
    table.title = f"Results of {model_name.upper()} on {difficulty.upper()} equations (TRAIN/VAL sets)."
    table.field_names = [
        "Equation",
        "Train R²",
        "Val R²",
...
def main() -> None:
    ...
    loader = SRSDLoader(difficulty=args.difficulty, splits=("train", "validation"))
    model = instantiate_algorithm(args.config)

    results = process_all_equations(model, loader)

    print_results_table(model.__class__.__name__, args.difficulty, results)

    save_expressions(
        results,
        Path(args.pickles_path) / args.difficulty / model.__class__.__name__.lower(),
    )
